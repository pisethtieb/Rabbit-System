...
//, 'eleven ', 'twelve ', 'thirteen ', 'fourteen ', 'fifteen ', 'sixteen ', 'seventeen ', 'eighteen ', 'nineteen '];
        ...
        if (contract.productType == 'monthlyFee') {
            ...
            data.product.installationFeeKh = toWords(contract.installationFee);
            data.product.trainingFeeKh = toWords(contract.trainingFee);
            data.product.installAndTrainingKh = toWords(contract.trainingFee + contract.installationFee);

            data.product.priceHead = contract.monthlyFee[0].headOffice;
            data.product.priceOffice = contract.monthlyFee[0].branch;
            data.product.installationFee = contract.installationFee;
            data.product.trainingFee = contract.trainingFee;
            data.product.installAndTraining = (contract.trainingFee + contract.installationFee);
